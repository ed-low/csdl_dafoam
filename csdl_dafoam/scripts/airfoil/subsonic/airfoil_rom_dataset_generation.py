...
# data = data_generator.load_h5(files[3])
# state_info = data_generator.state_info
# snapshots = np.array(np.concatenate([data["samples"]["states"][state_name]       for state_name in state_info.keys()], axis=0))
...
# reference_state = snapshots[:, 0]
# snapshots       = snapshots[:, 1:]

# x = 1 / scaling[:, None] * (snapshots - reference_state[:, None])

# print(x.shape)

# m = comm.allreduce(x.shape[0], op=MPI.SUM)
...
#     modes_n, s_n    = method_of_snapshots_distributed(x_n, comm=comm, weights_local=weights)
#     modes_np, s_np  = method_of_snapshots_distributed(x_np, comm=comm, weights_local=weights)

#     stable_ranks = []

#     for r in range(1, r_max):
#         modes_r_n   = modes_n[:, 0:r]
#         modes_r_np  = modes_np[:, 0:r]

#         manifold    = Grassmann(m, r, comm=comm, inner_product_weights=weights)

#         angles      = manifold.subspace_angles(modes_r_n, modes_r_np)
#         max_angle   = np.max(angles)

#         E_r_n       = np.sum(s_n[:r] ** 2) / np.sum(s_n ** 2)
#         E_r_np      = np.sum(s_np[:r] ** 2) / np.sum(s_np ** 2)

#         dE = np.abs(E_r_n - E_r_np)

#         print(f"snapshots {N:4d}, {N+1:4d}; modes {r}: angle {max_angle}, E {dE}") if rank == 0 else None

#         if (max_angle < angle_tol) and (dE < E_tol):
#             stable_ranks.append((N, r))
    
#     N = N + dN

# print(stable_ranks) if rank == 0 else None
