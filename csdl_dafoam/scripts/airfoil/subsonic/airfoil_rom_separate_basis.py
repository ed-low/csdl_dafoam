...
from csdl_dafoam.core.rom.rom_models import PerVariableLSPGModel
...
num_test_samples = 5   # LHS test points; reference point (all zeros) is always prepended
...
# ===============================
pod_modes_per_var, reference_fom_state, weights, scaling = \
    data_generator.load_per_variable_pod_modes(h5_path, n_modes_per_var=N_MODES_PER_VAR)

if rank == 0:
    for var, modes in pod_modes_per_var.items():
        print(f"\n  Loaded per-variable POD:  {var} {modes.shape}")
...
with csdl.experimental.mpi.enter_mpi_region(rank, comm) as mpi_region:
    ...
    # --- PerVariableLSPGModel ROM ---
    rom_model = PerVariableLSPGModel(
        dafoam_input_variables_group = dafoam_input_variables_group,
        pod_modes_per_var            = pod_modes_per_var,
...
for i in range(num_samples_with_ref):
    ...
    if rank == 0:
        ...
        print(f"{i:>4d}  {rom_drag[0]:>12.4e}  {fom_drag[0]:>12.4e}  {drag_rel[0]:>14.4e}  "
              f"{rom_lift[0]:>12.4e}  {fom_lift[0]:>12.4e}  {lift_rel[0]:>14.4e}")
