...
from csdl_dafoam.core.rom.rom_models import SplitBasisLSPGModel, NormalizationConfig
...
# ROM mode counts (None = use all available in h5)
N_MODES_FLOW = 20
N_MODES_TURB = 10
...
# ===============================
# region Split POD load
# ===============================
# NormalizationConfig — Mach from U0 / speed-of-sound at T0
gamma_air = 1.4
R_air     = 287.0
M_ref     = (flight_conditions_group.airspeed_m_s / ambient_conditions_group.a_m_s).value[0]
cv0       = R_air / (gamma_air - 1.0)

# nu_ref must match training (nuTilda0, not kinematic viscosity of air)
...
pod_modes_flow, pod_modes_turb, reference_fom_state, cell_volumes = \
    data_generator.load_split_pod_modes(h5_path, n_modes_flow=N_MODES_FLOW, n_modes_turb=N_MODES_TURB)

if rank == 0:
    print(f"\n  Loaded split POD:  flow {pod_modes_flow.shape},  turb {pod_modes_turb.shape}")
...
with csdl.experimental.mpi.enter_mpi_region(rank, comm) as mpi_region:
    ...
    # --- SplitBasisLSPGModel ROM ---
    rom_model = SplitBasisLSPGModel.from_norm_config(
        dafoam_input_variables_group = dafoam_input_variables_group,
        pod_modes_flow               = pod_modes_flow,
        pod_modes_turb               = pod_modes_turb,
    ...
    # NOTE: nuTilda DOFs are log-space amplitudes multiplied onto log-space modes, so this
    # expression gives a physically meaningless nuTilda field. For diagnostics, use
    # dafoam_instance_rom.getStates() after sim.run() to get the correctly-reconstructed state.
